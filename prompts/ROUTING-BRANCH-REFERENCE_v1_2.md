# ROUTING-BRANCH-REFERENCE · 사용자 발화 분기 경우의 수 종합 v1.2

> **이 문서의 성격**: 별도 신규 로직이 아니다. `ROUTER-PRIORITY_v1_0.md`
> (원래 설계 원칙)와 `AC-PRO-CORE_v1_8.txt`(§CORE·§INFO·§TAGS의 날짜별
...
>    `R2확정:id` 신호로 얹도록 배선했다. `AC-PRO-CORE_v1_8.txt`가 이
...
> `AC-PRO-CORE_v1_8.txt`가 정본이고, `ROUTER-PRIORITY_v1_0.md`가 그
...
| 이름표 | `ROUTER-PRIORITY_v1_0.md` | `AC-PRO-CORE_v1_8.txt` 인라인 주석 |
|---|---|---|
...
## §TAGS 전체 목록 (AC-PRO-CORE_v1_8.txt §TAGS 그대로, 참조용)
...
- `prompts/ROUTER-PRIORITY_v1_0.md` — 원래 설계 원칙(R0~R4, 요약
  ...
  전부 여기서 가져왔다.
- `prompts/AC-PRO-CORE_v1_8.txt` — 실제 운영 로직(정본). §CORE·§INFO·
