...
# 버전 변경 이력
# ─────────────────────────────────────────────────
# v1.1 (2026-07-23): "전문가 페르소나" 법적 근거 명시 작업(주피터님 지시)
#             반영. 기존에도 PHASE A-1이 "근거 법령"을 조사하고 PHASE E
#             체크리스트가 그 정확성을 확인하도록 돼 있었으나, 실제 산출물
#             (PHASE C 템플릿)에는 그 조사 결과를 담을 고정된 자리가 없어
#             — 실사 결과(SP_lawyer_v4_5.md 등 기존 산출물 확인) 조사는
#             됐는데 SP 본문에는 한 번도 기재되지 않는 드리프트가
#             발생해왔다. PHASE C 템플릿의 "0. IDENTITY" 바로 뒤에
#             구조화된 `## 0-1. LEGAL-BASIS` 섹션을 고정 산출물로 신설하고,
#             PHASE E 체크리스트 항목을 "정확한가"에서 "SP 본문에 실제로
#             기재돼 있는가+ 정확한가"로 구체화했다.
# 근거      : AGENT-COMMON_v3_40 §9 [전문가 AI 라우팅]이 "목록에 없으면
#             [SP_DRAFT_REQUEST]로 SP-Author에게 요청한다"고 이미 명시하고
#             있으나, 실제 `SP-AUTHOR_v1_15.md`는 정부기관 SP(관할계층·
...
호출되지 않는다 — `SP-AUTHOR_v1_15.md`의 승인 원칙(AGENT-COMMON §3-0 ③)을
그대로 계승한다.
...
4. **인접 페르소나 경계 확인**: `expert_routing_thought_experiment_100`·
   ...
   이 경우 PHASE UPDATE(기존 SP 경량 갱신, SP-AUTHOR_v1_15 §PHASE UPDATE와
...
## 0-1. LEGAL-BASIS (법적 근거 — 2026-07-23 신설, 생략 불가)
- 페르소나 유형: 전문가 페르소나 (Expert Persona)
- 근거 법령: PHASE A-1에서 조사한 개별 자격사법 또는 자격기본법의 정식
  명칭 (예: 변호사 페르소나 → 변호사법)
- 핵심 조항: 그 법의 직무·업무범위 조항(조번호 + 요지). 조번호는 국가
  법령정보센터 기준으로 조사 시점에 확인하고, 확신이 없으면 조번호
  없이 법령명만 기재하고 "조번호 재검증 필요"로 표시한다(지어내지
  않는다 — 이 문서 전체의 원칙과 동일).
- 업무 범위 요약: PHASE A-1의 업무범위를 1~2줄로
- 인간 전속 영역(면허 없이 불가): PHASE A-1의 인간 전속 영역과 동일 내용을
  여기서도 명시적으로 다시 확인(0. IDENTITY의 서술과 중복되어도 무방 —
  이 섹션은 법적 근거만 모아 한눈에 보기 위한 것)
- 자격 소관 부처: (예: 변호사 → 법무부)
- legal_basis_last_verified: PHASE A-1 조사 시점 날짜
...
- [ ] PHASE 0의 신규 필요성 판정 4개 항목이 실제로 타당한가(중복 아님을
      재확인)
- [ ] PHASE A-1의 법적 근거·업무범위·인간전속 영역이 **SP 본문 `## 0-1.
      LEGAL-BASIS`에 실제로 기재돼 있고**, 그 내용이 정확한가(가장 중요 —
      틀리면 C33-2/C23 하드룰이 잘못된 경계에 적용됨. 조사만 하고 SP
      본문에 반영되지 않는 드리프트가 과거에 실제로 있었으므로 "기재
      여부" 자체를 별도 확인 항목으로 둔다)
