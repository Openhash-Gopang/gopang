...
#   - #10·#15는 여전히 미해결 — 이유를 §7-9/§7-10에 명시(선행조건
#     미충족/저장소 밖 기존 이슈).
...
### 7-11. #10, #15 — 진행 상황 갱신 (4차 지시, 2026-07-13)

- **#10**: **Tier 1 목록 확정 완료** —
  `docs/ksco_schema_tier_classification_v1.md` v1.0(15개 중분류,
  주피터님 승인). 24/27(보건·법률)은 세분류 재분류를 실제
  AGENT-PROFESSION-24/27 작성 시점으로 명시적으로 미루기로 확정.
  **단, "L1 없이 L2만 있을 때 정상 동작하는가"라는 원래 검증 질문
  자체는 아직 안 풀렸다** — Tier1 목록은 확정됐지만 실제
  AGENT-PROFESSION-XX 파일 15개는 아직 한 줄도 작성되지 않았다.
  목록 확정은 이 검증의 **선행조건 해소**일 뿐, 검증 자체의 완료가
  아니다 — 그 구분을 흐리지 않는다.
...
- #10 — L1 Tier 1 목록 확정이 선행돼야 검증 가능(§6-11).
