...
### 7-11. #10, #15 — 여전히 미해결(이유 명시)

- **#10**: L1 Tier 1 목록(AGENT-PROFESSION-XX 후보군)이 아직 확정되지
  않아 "L1 없이 L2만 있을 때 정상 동작하는가"를 검증할 대상 자체가
  없다. Tier 1 확정이 선행조건이라 이번 3차 지시 범위에서 patch
  불가능하다 — 다음 라운드 과제로 남긴다.
