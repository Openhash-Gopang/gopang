...
#             AGENCY-AC-COMMON(공리 0·공리 1) → [본 SP] → SP-ORG-JPSPO →
#             {팀 SP 4개}
...
#                agency_id(org:JPSPO)는 src/worker/dept-task-handler.js
...
- 기관: 제주특별자치도장애인체육회
- 유형(type): `SPORTSCOUNCIL` (SP-ORGDIV-TEMPLATE §0-1 검증 완료 유형)
- 근거: 국민체육진흥법 + 대한장애인체육회 정관(시도장애인체육회는 광역단체 산하 지회로 편입)
...
AGENCY-AC-COMMON 공리 0에 따라, 이 AC는 **제주특별자치도장애인체육회라는 프로그램의 `main()` 함수**다. §3(COMPOSE)에 나열된 4개 과 SP는 `main()`이 호출하는 submodule이며, 각자 자신의 §INPUT_SCHEMA/OUTPUT_SCHEMA를 갖는다. 이 AC와 4개 submodule의 입출력 스키마는 최초 1회 정의로 고정되지 않는다 — 조직개편·법령 개정·신규 업무 발생 시 주기적으로 재검토·갱신한다.
