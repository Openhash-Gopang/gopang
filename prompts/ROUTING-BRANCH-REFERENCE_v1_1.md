# ROUTING-BRANCH-REFERENCE · 사용자 발화 분기 경우의 수 종합 v1.1

> **이 문서의 성격**: 별도 신규 로직이 아니다. `ROUTER-PRIORITY_v1_0.md`
> (원래 설계 원칙)와 `AC-PRO-CORE_v1_7.txt`(§CORE·§INFO·§TAGS의 날짜별
...
> v1.0에 전혀 없었다. 아래 "0.5단계"와 §참고 문서를 이번 개정에서
> 추가했다. 그 외 R1-AC 보강(trigger 아닌 "분야" 개념 매칭, 2026-08-06/07)도
> 2단계 항목에 반영했다.
>
> **정본(source of truth)은 이 문서가 아니다** — 실제 동작은 어디까지나
> `AC-PRO-CORE_v1_7.txt`가 정본이고, `ROUTER-PRIORITY_v1_0.md`가 그
...
| 이름표 | `ROUTER-PRIORITY_v1_0.md` | `AC-PRO-CORE_v1_7.txt` 인라인 주석 |
|---|---|---|
...
발화 수신
 ...
 │      └─ 실행요청 ↓
 │
 ├─ 0.5단계 [v1.1 신설] — 라우팅 힌트 사전필터 (routing-hint.js,
 │         2026-08-08). §CATALOG 전체(93개 이상 행)를 매번 훑기 전에
 │         후보를 미리 좁혀 [ctx] 블록에 "라우팅후보:id1,id2,..."로
 │         얹는 코드 단계 — AC 판단이 아니라 그 앞단의 전처리다.
 ...
 │      └─ 힌트는 참고용 후보 축소일 뿐 최종 판단이 아니다 — 아래
 │           2단계에서 AC가 여전히 R1-AC/R2-AC를 직접 적용하고,
 │           힌트가 없거나 명백히 안 맞으면 §CATALOG 표 전체로 복귀.
 │
 ├─ 1단계: 의도가 명확한가? (§CORE ①)
 ...
 │      │    │         의도가 명시됐는가?
 │      │    │      ├─ 제도·제3자 관점 → GWP 후보(기본값). [v1.1
 │      │    │      │    보강, 2026-08-06/07] 이 판정은 §CATALOG
 │      │    │      │    trigger 단어가 우연히 겹치는지가 아니라
 │      │    │      │    그 GWP의 "분야" 칸에 개념적으로 속하는지로
 │      │    │      │    한다 — 단, 그 사무를 전담하는 전용 GWP가
 │      │    │      │    따로 있으면 그쪽이 우선(예: 특허·재산가치
 │      │    │      │    평가처럼 전용 GWP가 없을 때만 klaw로 감).
...
| **AC 판단 이전의 코드 전처리(태그 아님)** | 라우팅 힌트 사전필터(0.5단계) — 후보를 좁힐 뿐 탭도 태그도 발생시키지 않는다 |

---

## §TAGS 전체 목록 (AC-PRO-CORE_v1_7.txt §TAGS 그대로, 참조용)
...
- `prompts/ROUTER-PRIORITY_v1_0.md` — 원래 설계 원칙(R0~R4, 요약
  ...
  전부 여기서 가져왔다.
- `prompts/AC-PRO-CORE_v1_7.txt` — 실제 운영 로직(정본). §CORE(119행~)·
  §INFO(309행~)·§TAGS(841행~) 참조. 이 문서의 R1-AC/R2-AC와 예외
  사례(ktelecom/kestate)는 전부 여기서 가져왔다.
- `src/gopang/ai/routing-hint.js` — 0.5단계 사전필터 오케스트레이션
  (2026-08-08 신설). system 캐시(DeepSeek Auto Prompt Caching) 보존을
  위해 후보 축소 결과를 system이 아닌 매 턴 user 메시지 앞 [ctx]
  블록에 얹는 방식을 택한 배경이 파일 상단 주석에 설명돼 있다.
- `src/gopang/ai/candidate-prefilter.js` — 0.5단계의 0단계(로컬
  trigger 매칭, LLM 미호출). 순수 문자열 부분일치 기반이라 리터럴
  trigger가 없는 패러프레이즈는 못 좁히는 한계가 파일에 명시돼 있다.
