...
async function handleKlawRelay(bodyText, env, corsHeaders, meta = null, ctx = null) {
  ...
  // 누락돼 있었다 — U7-3(PDV 중개 접근)·U8(GUID 색인 재조회)·신설된
  // U11(사용자 현황·성향 우선 파악) 등이 K-Law에는 적용되지 않고 있었다.
  // handleLLMRelay·handleBusinessRelay·handleGovRelay는 이미 둘 다
  // 주입하고 있었으므로, K-Law만 예외였던 것을 이번에 바로잡는다.
  const [universalIntegrity, universalCommon] = await Promise.all([
    _fetchUniversalIntegrity(), _fetchUniversalCommon(),
  ]);
...
// 않는다. 서버가 K-Public_common(매니페스트 최신본)을 GitHub에서 캐시 fetch해 항상
// system 맨 앞에 붙이고, 클라이언트가 보낸 messages 중 role:'system'은
// 전부 무시한다 — 클라이언트 코드가 실수(또는 고의)로 공통 규칙을
// 빠뜨리거나 조작할 수 있는 여지를 구조적으로 없앤다.
// ═══════════════════════════════════════════════════════════
// ★ 2026-07-20 정정: sp-catalog.json에 "K-Public_common" 키가 없어
// manifest 경로로 전환할 수조차 없던 상태였다(grep 0건 확인). 키를
// 신설하고 manifest 경로로 전환해 이 클래스의 버그를 근본적으로 없앤다.
...
// 2026-07-20 정정: 위 경고(v1_3→v1_5, v1_6 갱신 시 주석)가 세 번째로
// 재현될 뻔했다 — UNIVERSAL-common에 U11(사용자 현황·성향 우선 파악 원칙)을
// 신설하며 파일을 v1_7→v1_8로 올렸는데, 이 하드코딩 URL을 그대로 뒀다면
// K-Service·공공기관 AC·개인 AC 전부가 U11 없는 v1.7을 계속 받았을 것이다.
// UNIVERSAL-INTEGRITY가 이미 쓰고 있는 `_fetchByManifestKeyFromGithub()`
// (sp-catalog.json 경유)로 전환해 이 클래스의 버그를 구조적으로 제거한다.
...
async function _loadGovCommonChain(doCode) {
  ...
  const [kgov, overlayTemplate, overlayDataRaw, treeProtocol] = await Promise.all([
    ...
    fetch(GITHUB_RAW_BASE + '/prompts/gov-tree/00-common/JEJU-TREE-PROTOCOL_v1.0.md', { cache: 'no-cache' }).then(r => {
