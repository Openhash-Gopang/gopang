...
// 연산 시간(지연) 1초당 부가 과금(원). K-Law·K-Tax처럼 STEP 다단계 추론이라
// 호출당 지연이 긴 서브시스템일수록 예산을 자연히 더 빨리 소모하게 만든다.
// 서브시스템별로 가중치를 하드코딩하지 않아도 "실제로 오래 걸리고 많이 쓴
// 만큼" 공정하게 반영되는 자기조정 방식 — 계정을 오래 점유할수록(동시성
// 점유·rate limit 소모) 더 큰 비용으로 잡힌다는 취지도 함께 반영한다.
const KLAW_TIME_SURCHARGE_KRW_PER_SEC = 0.5;
...
// 대화 길이(토큰 비용) + 연산 시간(지연) 비례 과금 — 두 요소를 합산해 하나의
// KRW로 환산한다. 서브시스템 이름을 하드코딩하지 않는 범용 헬퍼이므로
// K-Tax 등 향후 다른 고부하 서브시스템의 relay에도 그대로 재사용할 수 있다.
function _computeLoadKRW(usage, priceTier, elapsedMs) {
  const tokenKRW = _deepseekUsageToKRW(usage, priceTier);
  const timeKRW  = (elapsedMs / 1000) * KLAW_TIME_SURCHARGE_KRW_PER_SEC;
  return { tokenKRW, timeKRW, total: tokenKRW + timeKRW };
}
...
async function handleKlawRelay(bodyText, env, corsHeaders, meta = null, ctx = null) {
  ...
  if (isStream) {
    const [forClient, forUsage] = res.body.tee();
    const usageTask = _parseUsageFromStream(forUsage).then(async usage => {
      const load = _computeLoadKRW(usage, priceTier, Date.now() - t0);
      ...
      await Promise.all([_klawSpendAdd(env, userKey, load.total), _klawSpendAdd(env, globalKey, load.total), recordStep()]);
    });
    if (ctx?.waitUntil) ctx.waitUntil(usageTask); else usageTask.catch(() => {});
    return new Response(forClient, { status:200, headers:{ ...corsHeaders, 'Content-Type':'text/event-stream', 'Cache-Control':'no-cache', 'X-Accel-Buffering':'no' } });
  }
