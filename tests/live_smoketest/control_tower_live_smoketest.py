...
신호에 집중한다)
  ...
  - 마크다운 볼드(**text**) 존재 → 위반(2026-08-09 신설, professor-ct-
    03-medicine 실사 재검증에서 "**문제 1**"이 LIVE-PASS로 오탐된 게
    발견돼 추가. x**2 같은 지수 표기 오탐, 한국어 조사 미탐 둘 다
    오프라인 유닛테스트로 사전에 잡고 보정했다 — 자세한 근거는 코드의
    BOLD_RE 주석 참고. __word__(밑줄 볼드)는 파이썬 dunder 표기와
    경계가 원천적으로 안 갈려서 이번엔 일부러 제외했다.
...
# 2026-08-09 신설 — professor-ct-03-medicine 실사 재검증에서 "**문제 1**"
# 같은 마크다운 볼드체가 LIVE-PASS로 오탐된 게 확인됐다. 원칙 문서
# (CONTROL-TOWER-PRINCIPLE §형식)는 "마크다운 제목·번호 매김 목록·불릿
# 목록"만 예시로 들었지만 "문서 서식을 쓰지 않는다"는 상위 규칙 자체는
# 볼드·인라인 코드도 포함한다고 해석하는 게 맞다 — 기존 3개 정규식은
# 전부 "목록형" 위반만 잡고 "단일 강조 서식"은 못 잡는 사각지대였다.
# ★ 오탐 방지(오프라인 유닛테스트로 실사 전 발견) — "x**2 + y**2" 같은
# 지수 표기(공학·수학 계열 페르소나에서 흔함)가 순진한 \*\*[^*]+\*\* 패턴에
# 걸린다("**2 + y**"를 볼드로 오판). 마크다운 볼드는 통상 여는/닫는 **가
# 알파벳·숫자·밑줄에 바로 붙지 않는다(앞뒤에 공백·문장부호·줄시작 등이 옴)
# — 반대로 지수 표기는 숫자에 바로 붙는다. lookaround로 그 경계를 구분한다.
# ★ 미탐 방지(오프라인 유닛테스트로 실사 전 발견, 2차) — 위 lookaround에
# \w를 그대로 쓰면 한국어 조사가 볼드 뒤에 바로 붙는 흔한 경우("**핵심
# 포인트**는")를 볼드가 아니라고 놓친다 — \w가 한글도 word char로 포함하기
# 때문이다. check_domain_keyword_collision.py에서 이미 한 번 발견됐던
# "한국어 조사에서 \b 경계 실패"와 동일 유형의 버그라 여기서도 같은 교훈을
# 적용한다: 경계 판정은 ASCII 영숫자·밑줄로만 좁힌다(한글은 경계로 안 침).
#
# ★ __word__(밑줄 볼드)는 이번엔 일부러 뺐다 — 실사로 관측된 실제 위반은
# 전부 **였고(medicine 사례), __는 파이썬 dunder 표기(`__init__` 등)와
# 여는/닫는 경계가 원천적으로 구분 불가능하다(오프라인 테스트로 확인:
# "__init__ 함수처럼"이 오탐됨 — 진짜 볼드와 dunder 둘 다 "문자열 시작 +
# ASCII 비단어문자 뒤"라는 동일한 경계 신호를 갖는다). 관측 근거 없는
# 패턴까지 욕심내다 오탐을 만들기보다, 확인된 ** 위반만 정확히 잡는다.
# __가 실제로 나타나면 그때 재검토(이 스크립트 상단 "한계" 절 참고).
