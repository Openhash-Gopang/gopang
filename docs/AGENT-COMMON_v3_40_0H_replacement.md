...
> ⚠ **2026-07-29 정정**: `AGENT-COMMON`은 이후 완전히 폐기되고
> `AC-SHADOW-CORE`/`AC-PRO-CORE`/`AC-FLASH-EXECUTOR`로 교체됐습니다
> (`sp-catalog.json`에 `AGENT-COMMON` 키 자체가 없음 — `docs/SP_TREE_GOVERNANCE_MANUAL_v1_0.md`
> §7 참조). 이 문서가 다루는 §0-H(기관·절차 오케스트레이션) 로직이 현재
> 어느 파일의 어느 섹션으로 이어졌는지는 확인되지 않았습니다 — 이 문서는
> **역사적 패치 기록으로만** 유효하며, 지금 AGENT-COMMON을 패치하려는
> 목적으로는 쓸 수 없습니다.
