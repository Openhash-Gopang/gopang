...
## 10. 후속 조치 (2026-07-23, 세 항목 순서대로 진행 — 주피터 지시)

### 10-1. B안 채택 — 가상 100원 한도 폐지
§5에서 미뤄뒀던 정책 결정: `FREE_QUOTA_KRW_LIMIT`를 100 → **0**으로 변경.
`_settleAiUsage`의 `remainingFree`가 항상 0이 되어, 이제 첫 턴부터
실사용량 전액이 실지갑 GDC로 청구된다 — "가입 직후 충전된 GDC 그
자체가 유일한 무료 예산"이라는 원 요청과 정확히 일치. 과거 KV
누적치(`hondi:free_spend`)는 그대로 남아있지만 한도가 0이라 더 이상
참조되지 않음 — 별도 삭제·마이그레이션 불필요. `FREE_QUOTA_ENFORCEMENT_ENABLED`
(개발 기간 차단 잠정 해제 플래그)는 건드리지 않음 — 이건 별도의
상용 출시 판단이라 이번 범위 밖으로 둠.

### 10-2. 관리자 재시도 엔드포인트
`POST /biz/admin/signup-bonus-retry` 신설 — `{secret, guids:[...]}`를
받아 각 guid에 대해 `_grantSignupBonus`를 재실행한다. 이 함수 자체가
멱등(KV 플래그 확인)이라, 이미 지급된 guid를 섞어 넣어도
`already_granted`로 안전하게 건너뛴다 — "실패한 것만 정확히 골라야"
하는 부담이 없다. 응답은 guid별 `{status: granted|already_granted|failed|error}`
배열. 별도 실패 로그 컬렉션은 만들지 않았음(로그에서 `SIGNUP_BONUS_MINT_FAILED`
태그로 guid를 찾아 이 엔드포인트에 넣는 수동 절차 — TODO로 남겨둠).

### 10-3. charge.html 실사용 검증 — 실제 버그 발견 및 수정
`gopang-wallet.js` 최신본을 직접 대조한 결과, **PC/미확인 기기에서는
지갑이 자동 생성되지 않는다**는 걸 확인함(2026-07-23 신설된 동작 —
"PC가 먼저 키를 만들어 진짜 계정 키와 어긋나는 사고" 방지 목적).
이 경우 `window.gopangWallet`은 계속 `null`로 남고
`window.gopangWalletNeedsSetup=true`만 세워진다 — 기존 charge.html은
이걸 "아직 로딩 중"과 구분 못 하고 5초 뒤 무조건 "잠시 후 다시
시도하세요"라는 **틀린 안내**를 보여줬다(실제로는 아무리 기다려도
채워지지 않는 상태). 지갑은 있는데 못 여는 경우(`gopangWalletLocked`)도
동일하게 놓치고 있었다.

수정: `waitForWallet()`이 `{guid, reason}` 형태로 반환하도록 바꿔
`setup_needed`/`locked`/`timeout`을 구분하고, 앞의 두 경우엔
`/auth/device-link.html`로 안내하는 링크를 즉시 보여주도록 했다
(webapp.html의 기존 지갑잠금 배너와 같은 목적지, 같은 관례). 이
gap(`gopang:wallet-setup-needed` 이벤트를 구독하는 페이지가 어디에도
없음)은 charge.html만의 문제가 아니라 **플랫폼 전체의 기존 공백**이라,
이번엔 charge.html만 고치고 다른 페이지는 손대지 않았다 — 전체
반영 여부는 별도 판단 필요.

### 변경 파일 (이번 후속)
- `worker.js` — `worker-gdc-charge-patch2.diff` (FREE_QUOTA_KRW_LIMIT=0,
  `_grantSignupBonus` 반환값 구조화, `handleSignupBonusRetry` 신설,
  라우트 등록)
- `charge.html` (gdc 저장소, 이미 배포된 파일 수정) — PC/잠금 지갑
  안내 추가
