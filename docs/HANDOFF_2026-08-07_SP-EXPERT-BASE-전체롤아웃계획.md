...
- v1.2 (2026-08-07, 주피터님 지시): **관제탑 원칙 미관철 결함 발견·수정**
  — 실사(의사 페르소나에 "머리가 아픈데 왜 이럴까?" 질의) 결과, STEP
  A(감별진단 등)가 요구하는 목록·표 구조를 "내부 추론용"과 "사용자
  출력용"으로 구분하는 조항이 어디에도 없어, STEP 0(정보수집) 완료
  전에 STEP A 전체를 조기 노출하는 위반이 확인됨(C35 적극적 보조
  원칙의 "케이스 분석을 능동적으로 하라"는 문구와의 해석 충돌이
  근본 원인으로 추정). `CONTROL-TOWER-PRINCIPLE_v1_0→v1_1.md`·
  `SP_common_guardrails v3_28→v3_29.md`(C50-1+) 동시 신설로 대응 —
  "STEP A류 산출물은 내부 추론 구조이지 출력 형식이 아니다"를 명문화,
  C35와 충돌 아님을 명시. 두 문서 모두 프로덕션 코드로 실제 조립
  결과에 포함되는 것까지 확인. **이 수정은 EXPERT_BASE·58개 배치와
  달리 개별 SP 파일을 건드리지 않고 상위 공통층 2개만 고쳐 62개
  전체(및 향후 세부분야 포함)에 한 번에 적용되는 구조** — 개별
  페르소나별 검증(특히 physician 외 다른 STEP A 목록형 SP들에서
  실제로 개선됐는지)은 아직 라이브로 재확인하지 않았다.
