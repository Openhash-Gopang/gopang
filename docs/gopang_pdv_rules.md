...
pdv_6w: {
  ...
  where: 'school.hondi.net',       // 문자열 → 객체로 변환 필요
...
function _getSvcRegistration(origin, svcId) {
  ...
  // Level 1: *.hondi.net 자동 — PDV 전송 불가
...
| `where` | object | `"school.hondi.net"` | `{ svc_url: "https://school.hondi.net" }` |
...
where: {
  svc_url: 'https://school.hondi.net',
  ...report.pdv_6w.where,  // "school.hondi.net" 문자열 스프레드 → 오류
},
...
### 6.4 업종/유형별 세부 데이터는 PDV가 아니라 Profile에 (2026-06-22 추가)

`profile_pdv_schema_plan_v1.md`(95개 업종 스키마 작업) 진행 중 확인한 원칙: **PDV는 업종별 구조화 데이터를 위한 슬롯을 따로 두지 않는다.** `what`에 무엇을 넣어도 `handlePdvReport`는 `what.summary` 문자열과 `content_hash`만 영구 저장하고 나머지는 버린다(§1의 "요약만 기록" 원칙이 코드로 강제돼 있음) — 그러니 업종별 구조화 필드(예: 한의원의 진료과목, 음식점의 메뉴)는 다음 중 하나로 처리한다:

1. **Profile에 저장** — `user_profiles.extra.public.industry_fields`(공개·반복조회용 데이터, 예: 메뉴·영업시간 같은 "지금 상태")
2. **하위 서비스 자체 DB에 저장** — PDV는 그 사실에 대한 짧은 요약(`what.summary`)과 무결성 증빙(`content_hash`)만 받는다(예: "한약 처방 1건, 환자동의 확인됨" + 처방전 원본의 해시)

PDV에 업종별 구조를 직접 넣으려는 시도는 십중팔구 의도와 다르게 조용히 버려지니, 새 하위 서비스를 등록하기 전에 "이 데이터가 반복 조회되는 현재 상태인가(Profile), 한 번 발생한 사건의 기록인가(PDV 요약 + 자체 DB)"부터 구분할 것.
...
```
Access to fetch at 'https://gopang-proxy...' from origin 'https://school.hondi.net'
...
// ❌ 수동 설정 — 브라우저가 무시
headers: {
  'Origin': 'https://school.hondi.net',  // 브라우저가 덮어씀
}

// ✅ 올바른 방법 — school.hondi.net 페이지에서 fetch 호출
// 브라우저가 자동으로 Origin: https://school.hondi.net 설정
...
const REGISTERED_SERVICES = {
  'school':    { level: 3, domain: 'school.hondi.net',    minAuth: 'L0', pdv: true },
  'klaw':      { level: 3, domain: 'klaw.hondi.net',      minAuth: 'L0', pdv: true },
  // 신규 서비스 추가:
  'myservice': { level: 3, domain: 'myservice.hondi.net', minAuth: 'L0', pdv: true },
};
```
...
K-School(school.hondi.net) 구현 과정에서 발생한 오류와 해결책을 정리합니다. 동일한 오류를 반복하지 않기 위해 상세히 기록합니다.
...
```javascript
// ❌ pdv_6w.where가 "school.hondi.net" 문자열일 때
where: {
  svc_url: 'https://school.hondi.net',
...
*school.hondi.net 구현 경험 기반 — 동일한 실수를 반복하지 말 것*
