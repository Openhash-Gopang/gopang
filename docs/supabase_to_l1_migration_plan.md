...
## 2026-07-05 추가 결정 — PDV 선호 요약(preference-summary)은 L1 네이티브로 신설

market의 상품/서비스 검색이 "나만의 AI비서"가 가진 PDV(나이·주소·성향·취향·
구매 이력)와 협업하도록 역할을 재정의하는 작업(같은 날짜, 별도 논의)에서,
AI비서가 PDV로부터 **카테고리 한정 취향 요약**을 만들어 market에 넘기는
새 엔드포인트가 필요해졌다. 사용자가 이 기능은 Supabase가 아니라 **L1
PocketBase에서 바로 진행**하기로 결정함(`agent_internal_sp`와 같은 패턴 —
마이그레이션 리스크 없이 처음부터 L1로 작성).

**문제**: 이 요약을 만들려면 PDV 원본 로그(현재 `pdv_log`, 전부 Supabase —
위 표 확인 결과 아직 이전 안 됨)를 읽어야 하는데, 어느 스토어를 읽을지에
따라 구현이 완전히 달라진다:
- (a) `pdv_log` 자체를 지금 L1로 옮기고, 신규 preference-summary 훅이 L1
  `pdv_log`를 직접 읽는 방식
- (b) `pdv_log`는 Supabase에 그대로 두고, L1 훅(엔드포인트만 L1에 둠)이
  내부적으로 Supabase `pdv_log`를 조회하는 하이브리드 방식(엔드포인트
  위치와 실제 데이터 위치가 다름)
- (c) preference-summary 전용의 새 L1 컬렉션을 만들어, 기존 pdv_log와는
  별도로 취향 관련 필드만 앞으로 여기 기록하기 시작하는 방식(과거 데이터는
  못 씀, 새로 쌓아야 함)

**아직 코드를 건드리지 않은 이유**: 위 세 가지 중 어느 것인지 사용자
확인이 필요하고, (a)/(c)의 경우 L1 `profiles`/신규 컬렉션의 실제 필드
스키마를 모르는 상태다(이 문서 상단의 "전제와 한계"와 같은 문제).
