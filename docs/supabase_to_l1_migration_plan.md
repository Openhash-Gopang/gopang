# Supabase → L1 PocketBase 이관 현황 트래커
(구 제목: "이전 로드맵" — 2026-07-19부로 진행상황판 성격이 강해져 제목 변경)

> **이 문서가 다루는 범위**: Supabase→L1 PocketBase 이관 현황 트래커(테이블별 진행 상태)
> **전체 문서 지도**: [MANUAL_INDEX.md](MANUAL_INDEX.md)

(2026-06-30 작성 — "Supabase 의존을 줄이고 L1로 최대한 이전" 요청에 따른 현황 정리.
2026-07-19 갱신: worker.js/record.js의 죽은 Supabase 코드 완전 제거 확인, 아래 표에
완료 상태 반영)

## 2026-07-19 갱신 요약
- **worker.js**: 전수 재확인 결과 살아있는 Supabase 참조(URL·키·fetch 호출) **0건**. 이 문서 작성 시점(6/30) 이후로도 여러 차례(7/7, 7/12, 7/14, 7/15) 이관이 이어져 사실상 완료 상태였고, 오늘 남은 건 죽은 코드(`SUPABASE_URL` 상수, 하드코딩 anon key, `_sbHeaders` 등 호출부 없는 잔재)뿐이었음 — 삭제 완료.
- **record.js**(`src/gopang/pdv/record.js`): `_recordPDV()`가 Supabase `pdv_log` 직접 INSERT 대신 `/pdv/report`(L1 `pdv_records`) 경유로 전환 완료. 아래 `pdv_log` 행 상태를 "완료"로 갱신.
- **gopang 저장소 전체**: `worker.js` 외 8개 파일(`ai_assistant.js`, `register.js`, `location.js`, `review.js`, `profile.js`, `webrtc-realtime.js`, `state.js`, `bulk_register.py`)에 아직 **살아있는** Supabase REST 호출이 남아있음(2026-07-19 조사) — 이번 세션 범위 밖, 별도 작업 필요.
...
| `pdv_log` | 10 | ✅ **완료(2026-07-19)** — `record.js`의 `_recordPDV()`가 `/pdv/report`(L1 `pdv_records`) 경유로 전환됨. `worker.js`의 `handlePdvReport`가 이미 이 경로를 정상 처리 중이었음(2026-07-14 이관) | 완료 |
...
> 등 이미 축약된 값)일 뿐이다 — `archive/gopang_db_manual_v2.md`(2026-07-19 폐기 이동) §3.4의 "PDV 6하원칙
