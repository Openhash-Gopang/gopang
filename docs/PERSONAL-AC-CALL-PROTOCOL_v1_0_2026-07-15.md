...
### `[PERSONAL_AC_EMERGENCY_BYPASS]` — 긴급 신호 시 §3~4 전체 절차 생략 (2026-07-15 신설)
...
특정 요청에 긴급 플래그가 있으면(예: 신고 접수 SP가 자체적으로 응급 신호를 감지한 경우), `[PERSONAL_AC_CALL]`은 다음처럼 다르게 처리된다:

```
[PERSONAL_AC_CALL: ..., emergency=true, emergency_reason={간단한 사유 분류}]
```

- `emergency=true`인 요청은 §4의 표준 왕복(②③④⑤)을 거치지 않는다 — **§6-1의 push 고지·수신확인 절차 자체를 생략**하고, 요청을 받은 개인 AC는 즉시 사람에게 직접 알린다(응급 상황에서는 "동의를 구하는 절차"가 아니라 "즉시 알려야 하는 통지"로 성격이 바뀐다).
- `PERSONAL_AC_TIMEOUT`이 적용되지 않는다 — TTL 만료라는 개념 자체가 이런 상황에는 맞지 않는다.
- 이 플래그를 남용해 일반 요청을 긴급으로 위장해 절차를 우회하는 걸 막기 위해, `emergency=true`는 §4의 `official_access_cert` 검증을 통과한 요청 중에서도 사전 등록된 긴급 유형(가정폭력·아동학대·실종 등 신고 접수 SP)만 발화할 수 있어야 한다 — 이 목록은 이 문서가 아니라 각 신고접수 SP의 소관이며, 이 프로토콜은 "긴급 플래그가 서면 절차를 생략한다"는 처리 규칙만 정의한다.
...
5. **긴급 유형 목록(`emergency=true` 발화 자격)이 아직 확정되지 않았다** — `[PERSONAL_AC_EMERGENCY_BYPASS]`가 어떤 신고접수 SP에 이 권한을 줄지는 이 문서가 정하지 않는다. 목록 확정과 각 SP의 실제 발화 조건 검증(오남용 방지)은 후속 작업.

---

*v1.0 (2026-07-15) — 최초 작성. SP-INTERCALL-PROTOCOL §7의 한계(개인정보·PDV 조회 위임 불가)를 메우는 신규 프로토콜. 기존 handlePdvQuery 동의 검증 인프라·`_sendPushToGuid`·AGENCY-AC-COMMON NOTICE 관례를 재사용하고, 새 동의 메커니즘은 만들지 않음.*
