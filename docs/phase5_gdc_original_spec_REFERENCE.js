/**
 * ⚠️ 참고용 원본 스펙 — 이대로 실행되지 않습니다.
 *
 * 이 파일은 gopang 저장소에서 2026-07-17에 삭제된
 * src/tests/network/phase5_network_gdc_privacy.test.js의 원본을 git
 * 히스토리에서 그대로 복원한 것입니다. GDC 관련 import(../../gdc/*.js)가
 * 이 저장소(gopang)에는 더 이상 존재하지 않는 경로를 가리킵니다 — 그
 * 파일들은 2026-07-15에 gdc 저장소로 이동했기 때문입니다.
 *
 * gdc 저장소 작업자는 이 파일의 G-01~08 테스트 케이스(로직/기대값)를
 * 참고해서, 이 저장소의 실제 src/gdc/*.js 파일들을 import하도록 경로만
 * 고쳐 새로 실행 가능한 테스트로 포팅해주시면 됩니다.
 * (docs/HONDI_DOMAIN_DEEP_TEST_DIRECTIVE_v1_0.md §4.5 참고)
 */
