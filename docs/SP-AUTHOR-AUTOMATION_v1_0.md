...
### 1-8. `unresolved_tag_signal` — 태그 해석 실패 (2026-07-14 신설)
**문제의식**: `expert-session.js`/`call-ai.js`가 `[EXPERT: personaId]`·`[GWP: serviceId]`
태그를 파싱했지만 `resolveExpertId()`/`getService()`가 매칭 실패하면(모델이 등록되지 않은
직군·서비스 이름을 태그로 냈을 때), 기존에는 콘솔 경고만 남기고 사용자에게도 운영자에게도
아무 신호가 안 갔다(사고실험 300건 분기표 검증 중 발견된 구조적 취약점). 이제
`_reportUnresolvedTag(kind, rawId, userText)`가 사용자 안내 버블과 함께 이 신호를
`POST /sp-author/queue`(`signal_source=expert_tag_resolution` 또는 `gwp_tag_resolution`,
`institution=rawId`, `priority=low`)로 큐잉한다.

**설계**: `search_miss_pattern`(1-3)과 원리는 같지만 그룹화가 더 단순하다 — `search_miss_pattern`은
사용자마다 다른 자연어 표현("분뇨 처리 신고" vs "축산 악취 민원")을 임베딩 유사도로
클러스터링해야 하지만, 이 신호의 `institution` 필드는 이미 정규화된 태그 raw_id 문자열
그대로라(모델이 표에서 고른 값이거나 표에 없는 슬러그) 문자열 완전일치 그룹화로 충분하다.
`tools/aggregate_unresolved_tags.py`가 매일 1회(sp-refresh-scheduler.yml에 통합) 실행되어:
  - 최근 7일 내 `status=queued`이고 `signal_source`가 `expert_tag_resolution`/
    `gwp_tag_resolution`인 레코드를 `institution`(raw_id) 기준으로 그룹화
  - 그룹 크기 **3건 이상**이면 최초 발생 레코드를 대표로 남기고 나머지는
    `status=duplicate`로 병합, 대표 레코드 `priority`를 `low→high`로 승격
  - 승격 시 `escalations` 레코드 생성(`reason=sp_draft_request`)

이 신호가 잦은 항목은 두 갈래로 갈린다 — (a) 등록해야 할 진짜 미등록 직군/서비스라면
SP-Author가 신규 작성 대상으로 채택, (b) 모델이 등록된 항목의 정확한 id를 못 찾아
헛다리를 짚는 경우(예: "회계사"를 "accountant" 대신 다른 문자열로 냄)라면 §9 표의
alias·구분주석을 보강해야 할 신호다 — 어느 쪽이든 사람이 판단해서 승인/거절한다는
원칙은 동일하다.

**선행 수정**: 이 설계가 의존하는 `POST /sp-author/queue/:id/status`가 지금까지
`status`만 patch할 수 있었다(`priority` 필드 patch 불가 — 이 설계문서의
