...
## 0. 저장소 토폴로지 (2026-07-25 신설 — 신규 개발자는 반드시 먼저 읽을 것)

> **이 문서(그리고 이 저장소 전체)는 `Openhash-Gopang/gopang` 하나만
> 다룹니다. 하지만 실제 배포된 사이트 중 최소 하나(`jeju.hondi.net`)는
> 별도 저장소 `Openhash-Gopang/jeju`에서 서빙됩니다.** 이 사실이 그동안
> 이 시스템맵을 포함한 개발자 온보딩 문서 어디에도 명시돼 있지 않았습니다
> (`docs/GOV_REGIONAL_AC_MANUAL_v1_0.md`에는 처음부터 명시돼 있었지만,
> 그 문서는 지방행정 AC라는 특정 기능을 찾아온 사람만 읽습니다 — 새
> 개발자가 온보딩 첫 문서로 여기부터 읽으면 두 번째 저장소가 존재하는지
> 전혀 알 길이 없었습니다).
...
| 저장소 | 역할 |
|---|---|
| `Openhash-Gopang/gopang`(이 저장소) | 핵심 로직·컴포넌트의 **원본**. `src/gopang/gov/gov-router.js`(지방행정 AC 조립 로직), `worker.js`(Cloudflare Worker 백엔드), `pages/regional-gov.html`·`gopang-app.js`(클라이언트) 등 이 시스템맵이 다루는 파일 전부가 여기 있다. |
| `Openhash-Gopang/jeju`(별도) | `jeju-router.js`가 `gopang` 저장소의 `gov-router.js`를 재수출하는 얇은 진입점(29줄)으로 축소돼 있고, `webapp.html`이 실제 사용자를 받는 배포 엔트리 — **현재 확인된 유일한 실배포 사이트**(`jeju.hondi.net`). |

**실무 영향**: `gopang` 저장소만 보고 "이 기능이 실제로 배포됐는지" 판단할
수 없습니다 — 특히 프론트엔드(`webapp.html` 쪽) 변경이 필요한 작업은
`jeju` 저장소도 함께 확인·수정해야 할 수 있습니다(예: `GOV_REGIONAL_AC_
MANUAL_v1_0.md` §4가 실제로 3개 저장소를 동시에 고친 사례를 다룹니다 —
`gopang`, `jeju`, 그리고 이 표에 없는 세 번째 저장소가 있을 수도
있다는 뜻이니, 새 기능을 배포까지 확인하려면 실제 배포 사이트에서 직접
동작을 확인하는 걸 원칙으로 하세요).

**이 표가 완전하다고 보장하지 않습니다** — 이번 조사(2026-07-25)는
`gopang` 저장소 내부에서 언급되는 외부 저장소 참조를 grep으로 찾은
것이라, 언급 자체가 없는 세 번째·네 번째 저장소가 존재할 가능성을
배제할 수 없습니다. 이 표에 없는 저장소를 발견하면 이 절에 추가해 주세요.
