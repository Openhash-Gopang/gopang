...
1. **STEP-1 (AC-PRO-CORE, `[EXPERT: professor]` 태깅)** —
   `scenarios_k12_professor_vs_teacher_20260809.json`(12건). 안전군(교사
   트리거 단어 없는 자연스러운 K-12 발화)과 위험군(발화에 "선생님"·"담임"·
   "정교사" 같은 teacher 트리거 문자열이 섞여 있지만 의미상 professor가
   정답인 함정 케이스)을 함께 넣었다 — 특히 no=6("학교 다녀와서 국어
   받아쓰기를 봐줄 선생님을 구하고 있어요")은 teacher의 트리거
   `'학교 선생님'`과 표면 문자열이 거의 겹쳐 오분류 위험이 가장 크다.
...
2. **STEP-2 (subject-gate, 리프 정밀화)** —
   `scenarios_k12_subject_gate_20260809.json`(18건: 매칭O·동의어O 7건 +
   매칭O·동의어X/근사치 8건 + 완전공백 3건). 완전공백 3건은
   ...
   무조건 FAIL — null-기대 시나리오를 표현할 수 없었음).
