...
2. **184건(gov-tree도 스텁) 처리 방침** — ✅ 2026-08-05 후속 세션에서
   재실행 가능한 루틴으로 완료: `tools/reconcile_org_profiles_govtree.mjs`
   (§2-3 알고리즘 그대로 재구현, `reconcile()` 함수 + `--emit-migration`
   CLI). gov-tree 실사가 끝나는 대로 이 스크립트를 다시 돌리면 된다 —
   자동 트리거(실사 완료 커밋마다 자동 재계산)까지는 아직 안 만들었음,
   현재는 사람이 필요할 때 수동 실행. 회귀 테스트(src/tests/reconcile-
   org-profiles-govtree.test.mjs)로 기존 28건 결과를 정확히 재현하는지
   검증됨.
3. **34건(미매칭) 처리 방침 결정** — ✅ 2026-08-05 후속 세션에서 결정·
   반영 완료. 34건을 실사해보니 무작위가 아니라 뚜렷한 두 카테고리였다:
   - **강원특별자치도(도청 + 산하 시·군, 18건)**: 다른 16개 도와 마찬가지로
     province/city 계층으로 매칭될 정상 후보인데, gov-tree 실사가 강원도
     쪽으로는 아직 진행되지 않았을 뿐. → **gov-tree 확장 대상**으로 계속
     `result.unmatched`에 남긴다. 실사가 진행되면 자연히 REAL로 옮겨간다
     (스크립트 재실행만 하면 됨, §7-2 참고).
   - **광역 교육청(16건, 예: 서울특별시교육청·경상남도교육청 등)**: 시·도·
     구·군과 관할·조직 구조가 근본적으로 다르다(학교급별·교육지원청 체계
     — gov-tree의 province/city 계층 개념 자체가 안 맞음). 주피터 결정
     (2026-08-05): **gov-tree로 편입하지 않는다** — org_profiles 쪽에서
     별도 트랙으로 처리할 대상이며, gov-tree 확장 후보 목록에 섞여
     실사 우선순위를 왜곡하지 않도록 별도 분류한다.
   - 구현: `tools/reconcile_org_profiles_govtree.mjs`의 `reconcile()`이
     이제 `result.unmatched`(강원도 등 gov-tree 확장 후보)와
     `result.unmatched_out_of_scope`(교육청 등 gov-tree 대상 아님)를
     분리해서 반환한다. 판별은 `org_name`이 `"교육청"`으로 끝나는지로
     기계적으로 한다(2026-08-05 기준 관측된 모든 사례가 이 패턴).
     회귀 테스트로 34=18+16 분리와 교육청 접미사 판별을 검증함
     (`src/tests/reconcile-org-profiles-govtree.test.mjs`).
   - **교육청 자체의 실제 처리 방법(별도 트랙이 구체적으로 뭘 의미하는지
     — 예: org_profiles에 남긴 채 `connected:false`로 둘지, 다른
     resolution_strategy를 신설할지)은 아직 미결정** — 이번 결정은
     "gov-tree 확장 대상에서 제외한다"까지이고, 교육청을 실제로 어떻게
     서비스할지는 후속 세션 과제로 남는다.
4. AC-PRO-CORE §CORE를 5단계(정부서비스 판별 → 입법/사법/행정·중앙/
   지방 조합 판단 → 오케스트레이션 계획 → 취합·전달 → PDV 기록)로
   재구성 — 이 문서의 §1에서 시작된 논의의 원래 목적. ✅ 2026-08-05
   후속 세션에서 완료: AC-PRO-CORE_v1_0.txt §ORCHESTRATION에 5단계
   대응표를 명문화(새 SP 불필요, `org_profiles.branch`를 K-Compose/
   K-Execute의 기존 로직 안에서 읽는 정도로 충분하다는 이전 세션
   결론 그대로 반영, AC 자신의 행동 변화는 없음을 명시).
