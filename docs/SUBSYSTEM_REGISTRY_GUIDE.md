...
gwp-registry.js 순회
        ...
        ▼
gdc.gopang.net/webapp.html?gwp=1 자동 호출
```
...
```javascript
{
  ...
  url:         'https://your-service.gopang.net/webapp.html',
...
```javascript
{
  ...
  name:        'K-Agriculture',
  category:    'ENV',
  url:         'https://farm.gopang.net/webapp.html',
  minAuth:     'L0',
  pdv:         true,
  priority:    7,
...
await fetch('https://gopang-proxy.tensor-city.workers.dev/pdv/report', {
  ...
  body: JSON.stringify({
    report: {
      ...
      where: { svc_url: 'https://farm.gopang.net/webapp.html' },
...
| 코드 | 영역 | 예시 서비스 |
|------|------|------------|
| `EMG` | 긴급·재난 | K-Emergency (911.gopang.net) |
...
```
https://farm.gopang.net/webapp.html
  ?gwp=1                          ← GWP 호출 표시
...
// 고팡으로 복귀 버튼
document.getElementById('back-btn').href = returnTo || 'https://gopang.net';
```
...
고팡 하위 시스템은 `*.gopang.net` 서브도메인을 사용합니다.  
도메인 설정은 `dev@gopang.net`으로 요청하십시오.

### GitHub Pages 배포 기준

```
DNS CNAME: your-service.gopang.net → openhash-gopang.github.io
...
```javascript
// *.gopang.net 서브도메인은 자동 허용
/^https:\/\/[a-z0-9-]+\.gopang\.net$/
```

별도 도메인(예: `fiil.kr`)은 `dev@gopang.net`으로 허용 요청.
...
A. 가능합니다. `dev@gopang.net`으로 도메인 허용 요청 후 등록하십시오. (예: `fiil.kr`)
...
**Q. 신규 카테고리가 필요하면?**  
A. `dev@gopang.net`으로 제안하십시오. DAWN 투표를 통해 추가됩니다.
...
| 용도 | 연락처 |
|------|--------|
| 등록 문의 | dev@gopang.net |
| 법률·컴플라이언스 | legal@gopang.net |
| 일반 문의 | hello@gopang.net |
| 긴급 보안 | security@gopang.net |
