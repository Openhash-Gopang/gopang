...
```js
{
  대상_식별자: {
    via: 'manifest' | 'url',
    key: 'sp-catalog.json의 키',   // via:'manifest'일 때
    url: '...',                  // via:'url'일 때
...
- **`via:'manifest'`**: `prompts/sp-catalog.json`(2026-07-09 이전 이름:
  `manifest.json` — W-16 참고)에 이미 안정적인 "총괄" SP
  ...
  먼저 sp-catalog.json에 안정 키를 등록해야 한다.
...
| `tax` 위임 대상 등록 | ⏳ sp-catalog.json에 안정 키 선등록 필요 |
