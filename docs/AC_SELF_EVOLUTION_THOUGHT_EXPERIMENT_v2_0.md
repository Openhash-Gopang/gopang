...
## E — job_ksco 자체 만료 체크 부재 (전 직종 공통) → 해결(2026-07-14)
...
**→ 해결하며 발견한 것**: AC-AUTHOR §7은 "이미 구현된 PDV 주기적
검토(v3.55)에 job_ksco.review_due를 편입한다"고 했는데, 그
`_buildPdvReviewContext()`를 다시 보니 **`hondi_profile_done==='1'`
(프로필 완성)이면 즉시 return하는 게이트가 있었다** — 즉 그건
"온보딩 미완료 사용자를 완료시키는" 용도였지, "완료된 지 오래된
프로필을 재확인시키는" 용도가 아니었다. 정확히 반대 대상 집단이다
(review_due가 지나는 건 프로필을 완성한 지 한참 된 사용자에게나
일어나는 일). 그래서 "편입"이 아니라 **독립적인 새 함수
`_buildJobKscoReviewContext()`**로 만들었다 — 프로필 완성 여부와
무관하게 동작, 자체 쿨다운(14일)으로 매 턴 안 물어보게 함,
`[JOB_KSCO_REVIEWED]` 태그로 확인 시도를 기록하고 `review_due`를
+30일 미룬다.
