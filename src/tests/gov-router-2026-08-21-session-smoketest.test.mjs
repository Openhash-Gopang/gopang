...
globalThis.fetch = async (url) => {
  ...
  if (u.endsWith('.json') || u.includes('.json?')) {
    ...
    if (u.includes('assembly-committee-master-data.json')) return { ok: true, text: async () => JSON.stringify({ 위원회목록: [] }) }; // 2026-08-21 신설 — 세션 스모크테스트가 국회위원회 매칭 경로를 태워서 크래시하던 것 방지(위원회 목록 없음으로 정직하게 폴백)
...
const CASES = [
  ...
  // ── WELFARE 권한 경계 자기인식 반영 + 시청 국 위임 라우팅 (2026-07-23 신설) ──
  // SP-DO-WELFARE_v1.3 — division-master-data.json(jeju-welfare 4개 과)의
  // 처분성_문구("개별 급여는 시청 OO과에서 확정된다")를 SP 본문에 반영한
  // 후속 검증. city-dept-master-data.json(welfare, jejusi/seogwipo)에 목적지
  // 데이터는 이미 있었으므로, 지역이 특정되면 라우터가 시청 국까지
  // 내려가는지를 확인한다(SP 본문 변경 자체는 라우팅에 영향 없음 — 이
  // 테스트는 "목적지가 실제로 연결돼 있는가"를 검증하는 것).
  { text: '기초생활수급 신청하고 싶어요', locationHint: '서귀포시', expectAgency: 'gov_do',
    expectContains: 'SP-CITYDEPT-seogwipo-welfare',
    note: '지역명 없음 — PDV 힌트(시 단위)만으로 시청 복지위생국까지 특정돼야 함' },
  ...
  // ══════════════════════════════════════════════════════════════
  // ★ 2026-08-21 신설 — GOV-TASK-904-GAP 세션 스모크테스트.
  // 이번 세션에 GOV_TASK 등록·재정의 판정을 내린 부서/기관으로, 부서명을
  // 직접 언급하지 않은 발화가 실제로 도달하는지 확인한다. 두 그룹으로 나눔:
  //   A) kw 직격(키워드 매칭만으로 도달 — LLM 불필요, 결정론적 검증)
  //   B) kw 미스 유도(자연스러운 구어체지만 kw 배열엔 없는 표현 — 반드시
  //      classifyFn(LLM) 폴백이 개입해야 정답에 도달. useClassify: true로
  //      실행하되, 로컬 mockClassify는 이 케이스들을 모른다 — GitHub
  //      Actions(DEEPSEEK_API_KEY)로 실제 LLM 호출 시에만 진짜 검증됨.
  //      로컬 실행 시엔 "LLM 폴백이 실제로 호출됐는가"만 확인하고
  //      "정답을 골랐는가"는 info로만 기록한다.
  // ══════════════════════════════════════════════════════════════
  ...
  { text: '휠체어를 타는데 이동할 때 쓸 수 있는 콜택시 같은 서비스가 있나요', useClassify: true,
    note: 'TRANSWEAK kw는 기관 정식명칭 하나뿐이라 100% 미스 예상 — _classifyDivisionFallback(LLM폴백) 호출 여부가 핵심 관찰 대상.' },
  { text: '우리 회사 제품 시험분석을 좀 맡기고 싶은데 어디로 가야 하나요', useClassify: true,
    note: 'JTP kw에 "시험분석" 자체가 없어 미스 예상 — LLM폴백 개입 여부 확인.' },
  { text: '대학생인데 등록금이나 생활비 도와주는 장학금 신청하고 싶어요', useClassify: true,
    note: 'JILES kw는 "장학금 지원"(공백 포함 정확 일치) 등 desc 원문 조각이라 자연스러운 표현과 미스 가능 — LLM폴백 확인.' },
  { text: '어린이집을 새로 운영하려고 하는데 급식 관련해서 신고할 데가 있나요', useClassify: true,
    note: 'CHILDMEAL kw에 "급식소 등록 관리" 등 원문 조각 — "신고할 데"류 자연 표현과 미스 가능성 확인.' },
  { text: '소상공인인데 대출받으려고 보증 좀 받고 싶어요', useClassify: true,
    note: 'JCGF kw가 "소기업"/"소상공인 대상 신용보증서 발급"(전체구 일치) — "보증 받고 싶다"처럼 짧은 표현은 미스 가능성 확인.' },
  { text: '임대주택에 들어가고 싶은데 어떻게 신청하나요', useClassify: true,
    note: 'JPDC kw는 "공공주택 개발" 등 desc 조각 — "임대주택 신청" 자연 표현과 미스 가능성, SP-DO-HOUSING(도청)과의 경합 여부도 관찰 대상.' },
];
