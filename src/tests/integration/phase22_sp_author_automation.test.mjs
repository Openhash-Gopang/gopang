...
describe('SPA-07: /sp-author/refresh-schedule POST — tier별 next_due_at 계산', () => {
  ...
  it('Phase 4 버그 수정 — tier만 재분류(refresh_completed 없음)해도 기존 next_due_at은 안 바뀜', async () => {
    const first = await call('/sp-author/refresh-schedule', { method: 'POST', body: { sp_id: 'SP-STABLE', tier: 'monthly' } });
    const originalDue = first.json.record.next_due_at;
    assert.equal(first.json.next_due_at_changed, true, '신규 레코드는 next_due_at이 설정돼야 함');

    // 다음날 스케줄러가 tier만 재계산해서 다시 호출 — 실제 갱신은 안 일어남
    const second = await call('/sp-author/refresh-schedule', { method: 'POST', body: { sp_id: 'SP-STABLE', tier: 'quarterly', call_count_30d: 3 } });
    assert.equal(second.json.next_due_at_changed, false, '단순 tier 재분류는 next_due_at을 안 건드려야 함(버그였던 부분)');
    assert.equal(second.json.record.next_due_at, originalDue, '마감일이 매번 미래로 밀려나면 due 판정이 영원히 안 됨');
    assert.equal(second.json.record.tier, 'quarterly', 'tier 자체는 정상적으로 갱신돼야 함');
  });

  it('refresh_completed:true면 실제 갱신 완료로 간주해 next_due_at을 다시 계산함', async () => {
    const first = await call('/sp-author/refresh-schedule', { method: 'POST', body: { sp_id: 'SP-REFRESHED', tier: 'weekly' } });
    const originalDue = first.json.record.next_due_at;

    const completed = await call('/sp-author/refresh-schedule', {
      method: 'POST', body: { sp_id: 'SP-REFRESHED', tier: 'weekly', refresh_completed: true },
    });
    assert.equal(completed.json.next_due_at_changed, true);
    assert.ok(completed.json.record.last_refreshed_at, 'refresh_completed 시 last_refreshed_at도 기록돼야 함');
  });
});
...
describe('SPA-09: /sp-author/queue — target_sp_id 기반 중복방지(Phase 4 버그 수정)', () => {
  it('서로 다른 target_sp_id의 update 요청은 institution/task가 같아도 병합되면 안 됨', async () => {
    // 정기갱신 신호는 institution이 비어있고 task 문구가 정형화돼 있어서
    // (모두 "정기 갱신 — ..." 동일 문구), 예전 로직(institution+task만
    // 보는 중복방지)이면 서로 다른 SP-A/SP-B 요청이 하나로 뭉개졌다.
    const a = await call('/sp-author/queue', {
      method: 'POST',
      body: { request_type: 'update', signal_source: 'refresh_schedule', target_sp_id: 'SP-A', task: '정기 갱신 — tier 스케줄 도래' },
    });
    const b = await call('/sp-author/queue', {
      method: 'POST',
      body: { request_type: 'update', signal_source: 'refresh_schedule', target_sp_id: 'SP-B', task: '정기 갱신 — tier 스케줄 도래' },
    });
    assert.equal(a.json.status, 'queued');
    assert.equal(b.json.status, 'queued', '다른 SP인데 merged_into_existing이 나오면 버그');
    assert.equal(db.sp_draft_requests.length, 2);
    assert.equal(db.sp_draft_requests[0].target_sp_id, 'SP-A');
    assert.equal(db.sp_draft_requests[1].target_sp_id, 'SP-B');
  });

  it('같은 target_sp_id로 재요청하면 여전히 병합됨(중복방지 자체는 유지)', async () => {
    const a = await call('/sp-author/queue', {
      method: 'POST', body: { request_type: 'update', signal_source: 'refresh_schedule', target_sp_id: 'SP-A', task: '정기 갱신' },
    });
    const a2 = await call('/sp-author/queue', {
      method: 'POST', body: { request_type: 'update', signal_source: 'refresh_schedule', target_sp_id: 'SP-A', task: '정기 갱신' },
    });
    assert.equal(a2.json.status, 'merged_into_existing');
    assert.equal(db.sp_draft_requests.length, 1);
    assert.equal(a2.json.record.id, a.json.record.id);
  });
});
