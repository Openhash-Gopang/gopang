...
function _showMandatoryBackupStep(key, handle) {
  return new Promise((resolve) => {
      ...
      <div style="background:#fff;border-radius:20px;padding:28px 22px;
        ...
        <div style="display:flex;gap:8px;margin-bottom:16px">
          <button id="_bm_copy" style="flex:1;padding:11px;border:1px solid #e5e7eb;border-radius:8px;
                  background:none;color:#1A73E8;font-weight:600;cursor:pointer;font-size:13px">
            복사
          </button>
        ...
        <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;margin-bottom:16px">
          <input type="checkbox" id="_bm_chk" style="width:16px;height:16px;margin-top:1px;accent-color:#1A73E8;flex-shrink:0">
    ...
    chk.addEventListener('change', () => {
      ...
      continueBtn.style.background = chk.checked ? '#1A73E8' : '#d1d5db';
...
function _showPhonePopup(resolve) {
  ...
  function _renderList(query = '') {
    ...
    listEl.innerHTML = entries.map(([key, c]) => `
      <div data-country="${key}"
           style="padding:10px 14px;cursor:pointer;
                  ...
                  ${key === selectedCountry ? 'background:#e8f0fe;' : ''}">
    ...
    listEl.querySelectorAll('[data-country]').forEach(el => {
      el.onmouseenter = () => el.style.background = '#e8f0fe';
      el.onmouseleave = () => el.style.background = el.dataset.country === selectedCountry ? '#e8f0fe' : '';
...
function _showNicknameStep({ ipv6, handle, e164, selectedCountry, val, overlay, resolve }) {
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;
      ...
      <label style="position:relative;display:inline-block;width:44px;height:26px">
        ...
        <span id="_is-public-slider"
              style="position:absolute;inset:0;background:#1A73E8;border-radius:13px;
                     cursor:pointer;transition:background .2s"
              onclick="this.style.background=document.getElementById('_is-public-chk').checked?'#d1d5db':'#1A73E8'">
  ...
  nickInput.focus();
  nickInput.addEventListener('focus', () => nickField.style.borderColor = '#1A73E8');
      ...
      try {
        ...
        if (total === 0) {
          ...
          hintEl.innerHTML =
            ...
            `<span style="color:#9ca3af">handle <b style="color:#1A73E8">${handle}</b>로 구분됩니다.</span>`;
        }
      } catch {
        hintEl.innerHTML = '';
      }
