/**
 * ai/manifest-loader.js — manifest.json 기반 SP 로더 (2026-07-09 신설)
 *
 * call-ai.js에 있던 _loadManifest()/_loadSpByKey()를 이 파일로 옮겼다.
 * 이유: expert-registry.js·expert-session.js(하드코딩된 파일 경로 방식)를
 * manifest.json 체계로 통합하면서, expert-session.js가 이 로더를 써야
 ...
 *
 * 이 분리 전까지 두 SP 로딩 체계(manifest.json 기반 vs expert-registry.js
