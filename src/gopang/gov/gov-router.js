...
// ★ 2026-08-03 수정 — NTS/KCS/MMA/PPS/PROSECUTION(국세청·관세청·병무청·
// 조달청·검찰청)은 policy-bodies(본청)와 agencies(지사) 양쪽에 다 있는
// 7개 기관 중 5개인데, 원래 등록돼 있던 키워드가 전부 아래
// _NAT_AGENCY_DOMAIN_KEYWORDS와 부분 문자열로 겹쳐서('종합소득세 신고'가
// tax 도메인의 '소득세'를 포함하는 식) 우선순위 가드(지사 우선)에
// 항상 걸려 본청 SP에 텍스트 매칭으로는 영원히 도달할 수 없었다
// (live-policy-bodies-smoketest.mjs 작성·실행 중 발견, 혼디 제1원칙 —
// K-Search로 기관을 정확히 특정해 호출한다 — 위반 사례). 각 5개 코드에
// 지사 키워드와 겹치지 않는 진짜 정책·본청 수준 키워드를 하나씩
// 추가했다(예: 국세청 '세법 해석 사전답변 신청'은 실제 국세청 본청
// 법령해석과 소관 업무이지 세무서 창구 업무가 아니다). 기존 키워드는
// 그대로 남겨뒀다 — 그쪽은 여전히(의도대로) 지사가 우선 처리한다.
