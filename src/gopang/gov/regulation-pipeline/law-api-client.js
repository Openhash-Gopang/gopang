...
export function createLawApiClient(ocId = 'test') {
  ...
  /**
   * 행정규칙 본문 조회.
   * @param {string} adminRuleId - 목록조회 결과의 "행정규칙ID"(또는
   *   행정규칙일련번호 — 검색 스니펫에서 두 필드가 같이 나와 어느 쪽을
   *   ID 파라미터로 써야 하는지 100% 확실치 않음. 상세링크 필드
   *   "/DRF/lawService.do?...&ID=..." 값을 그대로 쓰는 게 가장 안전).
