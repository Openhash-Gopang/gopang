...
P2P 채널에서 상대방 문의 시: 내 Profile 기반으로 대신 응대합니다.`,
  system_base: null,
  locationStr: '',
  // 혼디 제공 무료 기본 키(deepseek-default)의 티어 선택 — 'hondi-flash' | 'hondi-pro'
  // 본인 키를 등록하지 않은 사용자가 설정 화면에서 고를 수 있다(기본값 flash).
  hondiTier: 'hondi-flash',
};
...
export function saveSettings() {
  ...
  try {
    localStorage.setItem('gopang_cfg', JSON.stringify({
      ...
      providers: CFG.providers, hondiTier: CFG.hondiTier,
    }));
  } catch {}
...
export function loadSettings() {
  try {
    ...
    if (saved.hondiTier === 'hondi-flash' || saved.hondiTier === 'hondi-pro') {
      CFG.hondiTier = saved.hondiTier;
    }
    if (saved.apiKey || saved.geminiKey || CFG.providers.length) setAiActive(true);
  } catch {}
}

// ── 혼디 Flash/Pro 티어 선택 — 저장 버튼과 무관하게 즉시 반영·저장 ──
export function setHondiTier(tier) {
  if (tier !== 'hondi-flash' && tier !== 'hondi-pro') return;
  CFG.hondiTier = tier;
  try {
    const saved = JSON.parse(localStorage.getItem('gopang_cfg') || '{}');
    saved.hondiTier = tier;
    localStorage.setItem('gopang_cfg', JSON.stringify(saved));
  } catch {}
}
