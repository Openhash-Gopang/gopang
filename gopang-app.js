...
let _peer      = null;  // 현재 대화 상대 { guid, name, handle, entity_type }
let _rtcConn   = null;  // RTCPeerConnection
let _rtcChannel = null; // RTCDataChannel
let _signalPoll = null; // 폴링 타이머
let _pdvChatDB  = null; // IndexedDB 핸들
...
// ════════════════════════════════════════════════════════════
// P2P 채팅 — WebRTC DataChannel (OpenHash 철학)
...
async function _createOffer() {
  ...
  _rtcChannel = _rtcConn.createDataChannel('gopang-p2p', { ordered: true });
...
async function _handleOffer(sig) {
  ...
  await _rtcConn.setRemoteDescription(new RTCSessionDescription(JSON.parse(sig.payload)));
  const answer = await _rtcConn.createAnswer();
  await _rtcConn.setLocalDescription(answer);
  await _waitICE(_rtcConn);
  await _sendSignal('answer', _rtcConn.localDescription, sig.from_guid);
...
// 시그널 전송 → Worker /signal/send → Supabase webrtc_signals (60초 TTL)
...
// 시그널 폴링 (1.5초 간격)
function _startSignalPoll() {
  if (_signalPoll) clearInterval(_signalPoll);
  _signalPoll = setInterval(async () => {
    if (!USER_GUID) return;
    try {
...
async function _handleSignal(sig) {
  ...
  if (sig.type === 'ice') {
    if (_rtcConn) {
      try { await _rtcConn.addIceCandidate(new RTCIceCandidate(JSON.parse(sig.payload))); } catch(e) {}
    }
  }
...
async function _deleteSignal(id) {
  try { await fetch(`${PROXY}/signal/delete`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ id }) }); } catch(e) {}
}
async function _deleteMySignals() {
  try { await fetch(`${PROXY}/signal/delete`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ from_guid: USER_GUID }) }); } catch(e) {}
}
...
async function _loadChatHistory(peerGuid) {
  try {
    ...
    if (msgs.length) {
      ...
      msgs.forEach(m => appendBubble(m.dir === 'out' ? 'user' : 'peer', m.text, false,
