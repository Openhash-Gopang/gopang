# -*- coding: utf-8 -*-
"""
standalone: src/gopang/p2p/webrtc-realtime.js 를 직접 생성.
fix_realtime.py 로 webrtc.js 자동 패치가 어려울 경우 이 파일만 사용.
"""
import pathlib
...
if dest.exists():
    print(f"이미 존재: {dest} — 덮어쓰기 중단 (삭제 후 재실행)")
else:
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(REALTIME_JS, encoding='utf-8')
    print(f"생성 완료: {dest}")
